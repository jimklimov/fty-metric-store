/*  =========================================================================
    fty_metric_store_selftest.c - run selftests
...
// Tests for stable public classes:
    { "fty_metric_store_server", fty_metric_store_server_test, true, true, NULL },
...
// Now built only with --enable-drafts, so even stable builds are hidden behind the flag
    ...
    { "converter", NULL, true, false, "converter_test" },
    { "persistance", NULL, true, false, "persistance_test" },
    { "multi_row", NULL, true, false, "multi_row_test" },
...
{
    test_item_t *item;
    printf ("Running fty-metric-store selftests...\n");
    for (item = all_tests; item->testname; item++) {
        if (streq (item->testname, "private_classes"))
            continue;
        if (!item->subtest)
            item->test (verbose);
#ifdef FTY_METRIC_STORE_BUILD_DRAFT_API // selftest is still in draft
        else
            fty_metric_store_private_selftest (verbose, item->subtest);
#endif // FTY_METRIC_STORE_BUILD_DRAFT_API
    }
...
main (int argc, char **argv)
{
    ...
    for (argn = 1; argn < argc; argn++) {
        ...
        ||  streq (argv [argn], "-h")) {
            puts ("fty_metric_store_selftest.c [options] ...");
    ...
    if (test) {
        printf ("Running fty-metric-store test '%s'...\n", test->testname);
        if (!test->subtest)
            test->test (verbose);
#ifdef FTY_METRIC_STORE_BUILD_DRAFT_API // selftest is still in draft
        else
            fty_metric_store_private_selftest (verbose, test->subtest);
#endif // FTY_METRIC_STORE_BUILD_DRAFT_API
    }
    else
        test_runall (verbose);
