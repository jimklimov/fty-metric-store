/*  =========================================================================
    agent_metric_store_selftest.c - run selftests
...
{
    test_item_t *item;
    printf ("Running agent-metric-store selftests...\n");
    for (item = all_tests; item->test; item++)
        item->test (verbose);
...
main (int argc, char **argv)
{
    ...
    for (argn = 1; argn < argc; argn++) {
        ...
        ||  streq (argv [argn], "-h")) {
            puts ("agent_metric_store_selftest.c [options] ...");
    ...
    if (test) {
        printf ("Running agent-metric-store test '%s'...\n", test->testname);
        test->test (verbose);
    }
    else
        test_runall (verbose);
